...
namespace Legado.Core.Helps
{
    ...
    public abstract partial class JsExtensions
    {
        ...
        public async Task<StrResponse> StartBrowserAsync(string url, string title, bool refetchAfterSuccess)
        {
            if (browserService == null)
            {
                browserService = new BrowserService();
        ...
        public Task<StrResponse> StartBrowserAsync(string url, string title)
        {
            return StartBrowserAsync(url, title, true);
        }
    }
}
